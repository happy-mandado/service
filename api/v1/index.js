...
module.exports = (authConfig, service) => {
	...
	app.delete('/v1/users/:userId/products/:productId', function(req, res) {
		service.removeUserProduct(req.user.sub, req.productId)
			.then(product => res
				.status(200)
				.json(serializeProduct(product))
			)
	...
	app.post('/v1/users/:userId/lists', function(req, res) {
		service.closeUserList(req.user.sub, req.params.draftId, req.body.products)
			.then(list => res
				.status(201)
